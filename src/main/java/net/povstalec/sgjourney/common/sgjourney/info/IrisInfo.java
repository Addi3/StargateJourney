...
public class IrisInfo
{
	...
	private void playIrisSound(String action)
	{
		if(this.stargate == null || this.stargate.getLevel() == null || this.stargate.getLevel().isClientSide())
			return;

		ServerLevel level = (ServerLevel) this.stargate.getLevel();
		BlockPos pos = this.stargate.getBlockPos();
	...
	public short increaseIrisProgress()
	{
		...
		{
			boolean wasClosed = isIrisClosed();
			irisProgress++;

			setIrisState();

			if(irisProgress == 1 && oldIrisProgress == 0)
				playIrisSound("close"); // Sound when starting to close
		}
	...
	public short decreaseIrisProgress()
	{
		...
		if(hasIris() && irisProgress > 0)
		{
			...
			setIrisState();
			 ...
			 if(irisProgress == ShieldingState.MAX_PROGRESS - 1 && oldIrisProgress == ShieldingState.MAX_PROGRESS)
				playIrisSound("open"); // Sound when starting to open
		}
